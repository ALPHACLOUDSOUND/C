...
async def join(update: Update, context: CallbackContext):
    ...
    if not await is_user_in_group(update.message.from_user.id):
        await update.message.reply_text('You must be a member of the verification group to join the game.')
        return
...
async def ball(update: Update, context: CallbackContext):
    ...
    await update_scoreboard(update, chat_id)

async def update_scoreboard(update: Update, chat_id):
    ...
    if game_data[chat_id]['scoreboard_message_id']:
        try:
            await context.delete_message(chat_id=chat_id, message_id=game_data[chat_id]['scoreboard_message_id'])
        except Exception as e:
            logging.warning(f'Failed to delete message: {e}')
    
    # Send new scoreboard and pin it
    message = await context.send_message(chat_id=chat_id, text=scoreboard, parse_mode=ParseMode.MARKDOWN_V2)
    await context.pin_chat_message(chat_id=chat_id, message_id=message.message_id)
...
async def end_game(update: Update, context: CallbackContext):
    ...
    if user_id not in [BOT_OWNER_ID, *await get_group_admin_ids(update.message.chat_id)]:
    ...
    await update_scoreboard(update, chat_id)  # Update scoreboard one last time
...
async def ban_player(update: Update, context: CallbackContext):
    chat_id = update.message.chat_id
    user_id = update.message.from_user.id
    if chat_id not in game_data or game_data[chat_id]['status'] != 'playing':
        await update.message.reply_text('No game is currently running or the game has ended.')
...
async def get_group_admin_ids(chat_id):
    admin_ids = []
    try:
